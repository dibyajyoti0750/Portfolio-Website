...
export default function Navbar(): ReactElement {
  ...
  return (
    <nav className="w-full bg-white py-5 mb-5 md:mb-0 font-semibold">
      ...
      {showLinks && (
        <div className="flex flex-col gap-2 my-4 px-10 md:hidden text-center">
          {navLinks.map((item, idx) => (
            <a
              key={idx}
              href={item.link}
          ...
          <button className="flex items-center justify-center gap-2 bg-black text-white rounded mt-2 py-2 px-4 hover:bg-gray-900 transition cursor-pointer">
            Resume <Download className="w-4 h-4" />
          </button>
        </div>
      )}
